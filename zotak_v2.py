...
OPENROUTER_API_KEY = "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"
...
def analyze_scan_results(scan_type, output, cmd):
    ...
    try:
        ...
        if interesting_findings:
            ...
            # Query AI for suggestions - use dual mode for more accurate exploitation suggestions
            suggestions = query_ai(suggestion_prompt, f"Suggest exploits for {scan_type} findings", temperature=0.3, use_dual=True)
            if suggestions:
...
def ai_process_command(command):
    ...
    try:
        ...
        # Try to execute the command, falling back to other AI provider if needed
        result = attempt_command_with_provider(command, DEFAULT_PROVIDER, tried_providers)
        
        # If the command failed and we haven't tried all providers yet, try the other provider
        if not result and len(tried_providers) < 2:
            if GEMINI_AVAILABLE:
                fallback_provider = "gemini" if DEFAULT_PROVIDER != "gemini" else "openrouter"
                print(f"{Fore.YELLOW}[!] Command failed. Trying with {fallback_provider.capitalize()} API instead...{Style.RESET_ALL}")
                result = attempt_command_with_provider(command, fallback_provider, tried_providers)
            
        # Restore original provider setting
        DEFAULT_PROVIDER = original_provider
        
        if not result and len(tried_providers) > 0:
            print(f"{Fore.RED}[-] All AI providers failed to generate a working command.{Style.RESET_ALL}")
